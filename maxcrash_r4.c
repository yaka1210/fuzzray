...
void process(const char *filename) {
    ...
    // [R3 FIX] CWE-125: stack-buffer-overflow READ — null-terminate name
    hdr.name[15] = '\0';
    printf("Name: %s\n", hdr.name);
    ...
    // [R1+R2 FIX] CWE-190 ShiftOutOfBounds
    if (hdr.type > 1) {
        if (hdr.count >= 0 && hdr.offset > 0 && hdr.offset < 31) {
            if (hdr.count <= (INT32_MAX >> hdr.offset)) {
    ...
    // [R2 FIX] CWE-787: heap-buffer-overflow WRITE
    if (total >= (int32_t)sizeof(hdr.name)) {
        memcpy(buf, hdr.name, sizeof(hdr.name));
    } else {
        memcpy(buf, hdr.name, total);
    }

    // [R1 FIX] CWE-125: heap-buffer-overflow READ
    if (hdr.type > 2) {
        if (total > 0) {
    ...
    // [R2 FIX] CWE-415: double-free
    free(buf);
    buf = NULL;

    // [R3 FIX] CWE-476: use-after-free → теперь NULL deref, проверяем
    if (hdr.type > 5 && buf != NULL) {
        buf[0] = 'A';
    }
    ...
    // [R3 FIX] CWE-787: OOB write via negative index — проверка диапазона
    if (hdr.type > 10) {
        char local[32];
        memset(local, 0, 32);
        if (diff >= 0 && diff < 32) {
