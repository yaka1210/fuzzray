...
_RULES: list[tuple[re.Pattern[str], str, float]] = [
    ...
    (re.compile(
        r"(?:double[- ]free|attempting double[- ]free|double[- ]free or corruption)", re.I,
    ), "CWE-415", 0.95),
    ...
    # ASan SEGV with explicit access direction (no buffer-overflow label).
    # Check double-free / heap-corruption first: WRITE SEGV inside free()/Deallocate
    # means the second free() corrupted heap metadata → CWE-415 (weight beats CWE-787).
    (re.compile(r"SEGV on unknown address.{0,400}caused by.{0,30}WRITE.{0,800}in\s+(?:free|__interceptor_free\b|__asan::Allocator::Deallocate)", re.I | re.S), "CWE-415", 0.88),  # noqa: E501
    # Generic SEGV with direction — fallback when sanitizer cannot decode the address.
    (re.compile(r"SEGV on unknown address.{0,120}caused by.{0,30}WRITE", re.I | re.S), "CWE-787", 0.85),
    (re.compile(r"SEGV on unknown address.{0,120}caused by.{0,30}READ",  re.I | re.S), "CWE-125", 0.80),
    ...
    (re.compile(r"shift[- ]exponent\s+\S+\s+is\s+too\s+large|shift[- ]out[- ]of[- ]bounds|left shift of (?:negative value|\S+ by \S+ places)", re.I), "CWE-190", 0.9),  # noqa: E501
    ...
    (re.compile(r"member access within null pointer|reference binding to null pointer|member call on null pointer|downcast of null pointer|null pointer passed as argument|null pointer returned from function|applying (?:non-zero offset|zero offset) to null", re.I), "CWE-476", 0.9),  # noqa: E501

    # Pointer overflow / function type mismatch → CWE-476 (best fit in our set)
    (re.compile(r"applying \S+ offset .* produced null pointer|pointer overflow|calling function .* through pointer to incorrect function type", re.I), "CWE-476", 0.85),  # noqa: E501
    ...
    # MemorySanitizer allocation size (no CWE-457 — MSan not supported in this build)
    (re.compile(r"MemorySanitizer:\s*requested allocation", re.I), "CWE-190", 0.9),

    # UBSan: variable length array / unreachable → CWE-190 (best fit without MSan)
    (re.compile(r"variable length array bound evaluates to non-positive", re.I), "CWE-190", 0.7),
    ...
    (re.compile(r"__ubsan_handle_(?:float_cast_overflow|implicit_conversion)", re.I), "CWE-190", 0.75),
    (re.compile(r"__ubsan_handle_out_of_bounds", re.I), "CWE-125", 0.75),
    (re.compile(r"__ubsan_handle_(?:nonnull_arg|nonnull_return|nullability_arg|nullability_return|pointer_overflow|type_mismatch|function_type_mismatch|dynamic_type_cache_miss)", re.I), "CWE-476", 0.7),  # noqa: E501
...
def parse_sanitizer_output(text: str) -> tuple[dict[str, float], str | None]:
    ...
    overflow_m = _OVERFLOW_RE.search(text)
    if overflow_m:
        kind = overflow_m.group(1).lower()  # heap | stack | global
