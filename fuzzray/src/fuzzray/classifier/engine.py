...
def _apply_gdb(
    ...
    """Apply a GDB result to crash: update CWE distribution and fill crash fields.

    Mutates dist and crash in-place. Returns the detected sanitizer memory region.
    """
    ...
    if not san_dist:
        for k, v in _gdb_heuristics(gdb, signal_class).items():
            dist[k] = max(dist.get(k, 0.0), v)

    # Stack exhaustion (CWE-674) is not in our set — drop CWE so severity
    ...
    # Prefer absolute path from sanitizer stack (ASan/UBSan frames include the
    # full path; GDB 'at' gives only the basename).
...
def classify_one(
    ...
    if not no_replay and target is not None:
        ...
        if gdb is not None:
            sanitizer_region = _apply_gdb(crash, gdb, dist, signal_class, target)
...
def classify(
    ...
    for crash, gdb_res in zip(crashes, gdb_results, strict=True):
        ...
        if gdb_res is not None:
            sanitizer_region = _apply_gdb(crash, gdb_res, dist, signal_class, target)
