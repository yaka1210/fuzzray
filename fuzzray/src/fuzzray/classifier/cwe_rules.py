...
CWE_RECOMMENDATIONS: dict[str, str] = {
    "CWE-787": "Проверяйте границы буфера перед записью; используйте безопасные API с проверкой длины (strncpy, snprintf); компилируйте с флагами -D_FORTIFY_SOURCE=2 и -fstack-protector-strong.",
    "CWE-125": "Проверяйте индексы и длины перед чтением; используйте size_t и насыщающую арифметику; включите -fsanitize=address в CI.",
    "CWE-415": "Обнуляйте указатели после free() и проверяйте перед повторным освобождением; используйте обёртки с автоматическим обнулением; запускайте с -fsanitize=address.",
    "CWE-416": "Обнуляйте указатели после освобождения (free); используйте обёртки или arena-аллокаторы; запускайте с -fsanitize=address.",
    "CWE-119": "Аудит арифметики указателей и полей длины; предпочитайте абстракции span/slice; включите FORTIFY.",
    "CWE-476": "Проверяйте указатели перед разыменованием; превращайте неявные контракты в явные assert-проверки.",
    "CWE-190": "Используйте проверяемую арифметику (__builtin_*_overflow) или насыщение size_t; включите -fsanitize=undefined.",
    "CWE-369": "Проверяйте делитель перед операцией деления; включите -fsanitize=integer-divide-by-zero.",
    "CWE-457": "Инициализируйте все локальные переменные и поля структур; компилируйте с -Wuninitialized и MSan.",
    "CWE-681": "Проверяйте диапазон значений перед приведением типов (float→int, int64→int32); используйте явные проверки или safe_cast-обёртки; включите -fsanitize=undefined.",
    "CWE-134": "Никогда не передавайте пользовательские данные как форматную строку; всегда используйте литеральные форматные строки; включите -Wformat-security.",
}
