...
void process(const char *filename) {
    ...
    // [R1+R2 FIX] CWE-190 ShiftOutOfBounds — полный фикс
    if (hdr.type > 1) {
    ...
    // [R2 FIX] CWE-787: heap-buffer-overflow WRITE — проверка размера
    ...
    // [R2 FIX] CWE-415: double-free — убран повторный free
    free(buf);
    buf = NULL;
